...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.view-product-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            // Empêche la soumission du formulaire ou autre action par défaut
            e.preventDefault(); 
            
            // Redirige l'utilisateur vers la page vetement.html
            window.location.href = 'vetement.html';
        });
    });

    // Cible tous les éléments de la collection (qui agissent comme des boutons)
    document.querySelectorAll('.collection-item').forEach(item => {
        item.addEventListener('click', (e) => {
            // Utilise .closest() pour s'assurer que nous ciblons bien le conteneur principal
            // si le clic tombe sur l'image ou le texte.
            const targetElement = e.target.closest('.collection-item');
            
            if (targetElement && targetElement.dataset.target) {
                // Redirige vers l'URL spécifiée dans data-target
                window.location.href = targetElement.dataset.target;
            }
        });
    });

    // Cible tous les éléments de la collection (qui agissent comme des boutons)
    document.querySelectorAll('.collection-item-2').forEach(item => {
        item.addEventListener('click', (e) => {
            // Utilise .closest() pour s'assurer que nous ciblons bien le conteneur principal
            // si le clic tombe sur l'image ou le texte.
            const targetElement = e.target.closest('.collection-item');
    ...
    // Cible tous les éléments de la collection (qui agissent comme des boutons)
    document.querySelectorAll('.collection-item-3').forEach(item => {
        item.addEventListener('click', (e) => {
            // Utilise .closest() pour s'assurer que nous ciblons bien le conteneur principal
            // si le clic tombe sur l'image ou le texte.
            const targetElement = e.target.closest('.collection-item');
...
// ====== UGC SLIDER (Simple scroll horizontal) ======
const ugcSlider = document.querySelector('.ugc-slider');
if (ugcSlider) {
    ugcSlider.addEventListener('wheel', e => {
        e.preventDefault();
        ugcSlider.scrollLeft += e.deltaY;
    });
}
...
if (productsContainer && btnLeft && btnRight) {

    const gap = 20; // Correspond au gap défini en CSS
    const card = productsContainer.querySelector('.product-card');
    const cardWidth = card.offsetWidth + gap; // largeur + gap
    let scrollPos = 0;

    // Fonction pour mettre à jour la translation
    function updateSlider() {
        productsContainer.style.transform = `translateX(-${scrollPos}px)`;
    }

    // Bouton gauche
    btnLeft.addEventListener('click', () => {
        scrollPos -= cardWidth; // défiler d’une carte
        if (scrollPos < 0) scrollPos = 0; // bloque au début
        updateSlider();
    });

    // Bouton droite
    btnRight.addEventListener('click', () => {
        const maxScroll = productsContainer.scrollWidth - productsContainer.parentElement.offsetWidth;
        scrollPos += cardWidth; // défiler d’une carte
        if (scrollPos > maxScroll) scrollPos = maxScroll; // bloque à la fin
        updateSlider();
    });
}
